...
function SuccessOrderModal() {
    const { successModalOpen } = useStore((state) => state);
            ...
            <div className="relative w-full max-w-md max-h-full">
                <div className="relative bg-white rounded-2xl shadow px-10 py-5">
                    <div className="p-8 flex flex-col justify-center items-center">
                        ...
                        <Link to="/" className="w-full py-2.5 bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm text-center">
                            <button type="button" className="text-white">OK</button>
                        </Link>
                    </div>
                </div>
            </div>
